...
export const Header: React.FC<HeaderProps> = ({ alAbrirMovil }) => {
  ...
  return (
    <header className="h-18 bg-white/95 backdrop-blur-md border-b border-slate-200/80 sticky top-0 z-20 px-4 sm:px-8 flex items-center justify-between transition-all">
      ...
      {/* Indicador de modo + Avatar y datos de usuario */}
      <div className="flex items-center gap-4">
        {modoDemostracion && (
          <span className="hidden md:inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-sky-50 text-sky-700 border border-sky-200/60">
            <Sparkles className="w-3.5 h-3.5 text-sky-500" />
            <span>Modo Académico</span>
          </span>
        )}
